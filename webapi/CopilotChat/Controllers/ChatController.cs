...
using CopilotChat.Skills.Presentation;
...
public class ChatController : ControllerBase, IDisposable
{
    ...
    private async Task RegisterPlannerSkillsAsync(CopilotChatPlanner planner, OpenApiSkillsAuthHeaders openApiSkillsAuthHeaders, ContextVariables variables)
    {
        ...
        var disablePresentationPlugin =  openApiSkillsAuthHeaders.DisablePresentationPlugin;
        if (!string.Equals(disablePresentationPlugin, "false", StringComparison.OrdinalIgnoreCase))
        {
            planner.Kernel.ImportSkill(new PresentationPlugin(), "Presentation");

            var skillsDirectory = Path.Combine(System.IO.Directory.GetCurrentDirectory(), "CopilotChat", "Skills");
            planner.Kernel.ImportSemanticSkillFromDirectory(skillsDirectory, "SummerizePlugin");
        }
